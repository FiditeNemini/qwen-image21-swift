// Weight-free probe: mlx's Winograd conv2d path at Qwen-Image-2.1 VAE shapes, raw vs the
// WinogradFreeConv2d route (Sources/QwenImage21/WinogradFreeConv2d.swift), both against the CPU conv.
//
...
final class WinogradProbeTests: XCTestCase {
    ...
    func testAEConvShapes() throws {
        // (C, O, spatial): decoder conv_in / 1152-ch mid, the 576/288-ch stages, encoder conv_out.
        for (c, o, s) in [(64, 1152, 64), (1152, 1152, 64), (1152, 576, 128), (576, 288, 256), (768, 128, 64)] {
        ...
        let small = WinogradFreeConv2d(inputChannels: 288, outputChannels: 144, kernelSize: 3, padding: 1)
        let z = MLXRandom.normal([1, 64, 64, 288])
        XCTAssertFalse(
            WinogradFreeConv2d.takesWinograd(
